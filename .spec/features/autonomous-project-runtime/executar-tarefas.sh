...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo autonomous-project-runtime
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/autonomous-project-runtime/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-autonomous-project-runtime-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-autonomous-project-runtime"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/autonomous-project-runtime/executar-tarefas.sh --faixa $1"
...
T-027 — "Criar o assistente interativo de setup"
  critérios/refs: AC-042 (Setup descobre projeto e valida pré-requisitos), AC-043 (Setup oferece configuração completa com defaults seguros), AC-044 (Caminho feliz exige apenas escolhas essenciais), AC-045 (Instalação não interativa continua determinística), AC-049 (Backend remoto respeita isolamento estrito), AC-053 (Segredos permanecem apenas em memória ou ambiente)
  arquivos permitidos (e seus testes): src/cli/setup.ts, src/cli/prompts.ts, src/cli/index.ts, src/config/project-config.ts, tests/integration/setup.test.ts, tests/fixtures/setup-answers.ts
  mensagem de commit: "T-027 autonomous-project-runtime: Criar o assistente interativo de setup"
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/autonomous-project-runtime/executar-tarefas.sh --gate"
      exit 0
    fi
