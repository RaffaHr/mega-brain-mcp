#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano p6-git-temporal-symbol-intelligence` em 2026-08-27 18:33
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo p6-git-temporal-symbol-intelligence
set -u
set -o pipefail

RUN_ID='mega-brain-mcp-p6-git-temporal-symbol-intelligence-mtbv2wj0'
FEATURE='p6-git-temporal-symbol-intelligence'
BASE_BRANCH='spec/p6-git-temporal-symbol-intelligence'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/p6-git-temporal-symbol-intelligence/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p6-git-temporal-symbol-intelligence-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p6-git-temporal-symbol-intelligence"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/p6-git-temporal-symbol-intelligence/executar-tarefas.sh --faixa $1"
...
# ── faixa-1: T-058 T-059 T-060 ──
executar_faixa_1() {
  local WT="$WT_BASE-faixa-1"
  preparar_worktree 'faixa-1' 'spec/p6-git-temporal-symbol-intelligence-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-058' 'Você executa UMA tarefa da feature "p6-git-temporal-symbol-intelligence" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p6-git-temporal-symbol-intelligence/spec.md, .spec/features/p6-git-temporal-symbol-intelligence/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-058 — "Implementar mineração de histórico por símbolo com Git Pickaxe"
  critérios/refs: AC-092 (Mineração de histórico de símbolos via Git Pickaxe)
  arquivos permitidos (e seus testes): src/adapters/git/history.ts, src/tools/brain-history.ts, test/p6-git-temporal-symbol-intelligence.spec.test.js
  mensagem de commit: "T-058 p6-git-temporal-symbol-intelligence: Implementar mineração de histórico por símbolo com Git Pickaxe"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-059' 'Você executa UMA tarefa da feature "p6-git-temporal-symbol-intelligence" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p6-git-temporal-symbol-intelligence/spec.md, .spec/features/p6-git-temporal-symbol-intelligence/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-059 — "Integrar timeline ancorada do AgentMemory no brain_history"
  critérios/refs: AC-093 (Linha do tempo de episódios ancorada no brain_history)
  arquivos permitidos (e seus testes): src/server/application.ts, src/tools/brain-history.ts, test/p6-git-temporal-symbol-intelligence.spec.test.js
  mensagem de commit: "T-059 p6-git-temporal-symbol-intelligence: Integrar timeline ancorada do AgentMemory no brain_history"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-060' 'Você executa UMA tarefa da feature "p6-git-temporal-symbol-intelligence" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p6-git-temporal-symbol-intelligence/spec.md, .spec/features/p6-git-temporal-symbol-intelligence/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-060 — "Implementar contagem de churn e alerta de risco por símbolo no brain_change_context"
  critérios/refs: AC-094 (Alerta de risco por símbolo com alto churn histórico)
  arquivos permitidos (e seus testes): src/orchestration/change-context.ts, src/tools/brain-change-context.ts, src/server/application.ts, test/p6-git-temporal-symbol-intelligence.spec.test.js
  mensagem de commit: "T-060 p6-git-temporal-symbol-intelligence: Implementar contagem de churn e alerta de risco por símbolo no brain_change_context"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/p6-git-temporal-symbol-intelligence-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-058 T-059 T-060
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/p6-git-temporal-symbol-intelligence/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-058, T-059, T-060"
  echo
