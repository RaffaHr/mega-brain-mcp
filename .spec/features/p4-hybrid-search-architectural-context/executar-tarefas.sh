#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano p4-hybrid-search-architectural-context` em 2026-08-27 18:31
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo p4-hybrid-search-architectural-context
set -u
set -o pipefail

RUN_ID='mega-brain-mcp-p4-hybrid-search-architectural-context-mtbuzsgx'
FEATURE='p4-hybrid-search-architectural-context'
BASE_BRANCH='spec/p4-hybrid-search-architectural-context'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/p4-hybrid-search-architectural-context/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p4-hybrid-search-architectural-context-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p4-hybrid-search-architectural-context"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/p4-hybrid-search-architectural-context/executar-tarefas.sh --faixa $1"
...
# ── faixa-1: T-052 T-053 T-054 ──
executar_faixa_1() {
  local WT="$WT_BASE-faixa-1"
  preparar_worktree 'faixa-1' 'spec/p4-hybrid-search-architectural-context-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-052' 'Você executa UMA tarefa da feature "p4-hybrid-search-architectural-context" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p4-hybrid-search-architectural-context/spec.md, .spec/features/p4-hybrid-search-architectural-context/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-052 — "Implementar tabela virtual FTS5 e busca lexical BM25 no ProvenanceRepository"
  critérios/refs: AC-086 (Indexação FTS5 e busca lexical ponderada por BM25)
  arquivos permitidos (e seus testes): src/provenance/migrations.ts, src/provenance/repository.ts, test/p4-hybrid-search-architectural-context.spec.test.js
  mensagem de commit: "T-052 p4-hybrid-search-architectural-context: Implementar tabela virtual FTS5 e busca lexical BM25 no ProvenanceRepository"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-053' 'Você executa UMA tarefa da feature "p4-hybrid-search-architectural-context" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p4-hybrid-search-architectural-context/spec.md, .spec/features/p4-hybrid-search-architectural-context/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-053 — "Integrar canal lexical local no RRF Ranker e application handlers"
  critérios/refs: AC-087 (Fusão de ranking híbrido RRF integrando canal lexical)
  arquivos permitidos (e seus testes): src/orchestration/ranking.ts, src/orchestration/router.ts, src/server/application.ts, test/p4-hybrid-search-architectural-context.spec.test.js
  mensagem de commit: "T-053 p4-hybrid-search-architectural-context: Integrar canal lexical local no RRF Ranker e application handlers"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-054' 'Você executa UMA tarefa da feature "p4-hybrid-search-architectural-context" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p4-hybrid-search-architectural-context/spec.md, .spec/features/p4-hybrid-search-architectural-context/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-054 — "Injetar visão geral de arquitetura do CRG no Context Builder em queries de arquitetura"
  critérios/refs: AC-088 (Injeção de resumo arquitetural nativo em consultas de arquitetura)
  arquivos permitidos (e seus testes): src/server/application.ts, src/orchestration/context-builder.ts, test/p4-hybrid-search-architectural-context.spec.test.js
  mensagem de commit: "T-054 p4-hybrid-search-architectural-context: Injetar visão geral de arquitetura do CRG no Context Builder em queries de arquitetura"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/p4-hybrid-search-architectural-context-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-052 T-053 T-054
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/p4-hybrid-search-architectural-context/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-052, T-053, T-054"
  echo
