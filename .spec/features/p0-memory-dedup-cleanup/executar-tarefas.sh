#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano p0-memory-dedup-cleanup` em 2026-08-27 14:37
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo p0-memory-dedup-cleanup
set -u
set -o pipefail

RUN_ID='mega-brain-mcp-p0-memory-dedup-cleanup-mtbmn0l3'
FEATURE='p0-memory-dedup-cleanup'
BASE_BRANCH='spec/p0-memory-dedup-cleanup'
ENGINE='C:\Users\raphael.moreira\.claude\skills\onp-spec-driven\scripts\onp-spec.mjs'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/p0-memory-dedup-cleanup/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p0-memory-dedup-cleanup-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p0-memory-dedup-cleanup"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/p0-memory-dedup-cleanup/executar-tarefas.sh --faixa $1"
...
executar_seq_T_040() {
  info 'sequencial T-040 — Eliminar criação de memórias sintéticas em reforço e conflito'
  if rodar_tarefa seq 'T-040' 'Você executa UMA tarefa da feature "p0-memory-dedup-cleanup" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p0-memory-dedup-cleanup/spec.md, .spec/features/p0-memory-dedup-cleanup/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-040 — "Eliminar criação de memórias sintéticas em reforço e conflito"
  critérios/refs: AC-070 (Reforço de memória sem geração de registro textual duplicado)
  arquivos permitidos (e seus testes): src/server/application.ts, src/tools/brain-learn.ts, test/p0-memory-dedup-cleanup.spec.test.js
  mensagem de commit: "T-040 p0-memory-dedup-cleanup: Eliminar criação de memórias sintéticas em reforço e conflito"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-040 p0-memory-dedup-cleanup: Eliminar criação de memórias sintéticas em reforço e conflito (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/p0-memory-dedup-cleanup/executar-tarefas.sh --seq T-040"
  FALHAS="$FALHAS T-040"
  return 1
}
...
executar_seq_T_041() {
  info 'sequencial T-041 — Registrar supersessão exclusivamente na proveniência e metadados'
  if rodar_tarefa seq 'T-041' 'Você executa UMA tarefa da feature "p0-memory-dedup-cleanup" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p0-memory-dedup-cleanup/spec.md, .spec/features/p0-memory-dedup-cleanup/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-041 — "Registrar supersessão exclusivamente na proveniência e metadados"
  critérios/refs: AC-071 (Rastreabilidade de supersessão sem poluição do espaço vetorial)
  arquivos permitidos (e seus testes): src/server/application.ts, src/tools/brain-learn.ts, src/provenance/repository.ts, test/p0-memory-dedup-cleanup.spec.test.js
  mensagem de commit: "T-041 p0-memory-dedup-cleanup: Registrar supersessão exclusivamente na proveniência e metadados"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-041 p0-memory-dedup-cleanup: Registrar supersessão exclusivamente na proveniência e metadados (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/p0-memory-dedup-cleanup/executar-tarefas.sh --seq T-041"
  FALHAS="$FALHAS T-041"
  return 1
}
...
executar_seq_T_042() {
  info 'sequencial T-042 — Implementar truncamento seguro de blocos e deduplicação no Context Builder'
  if rodar_tarefa seq 'T-042' 'Você executa UMA tarefa da feature "p0-memory-dedup-cleanup" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p0-memory-dedup-cleanup/spec.md, .spec/features/p0-memory-dedup-cleanup/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-042 — "Implementar truncamento seguro de blocos e deduplicação no Context Builder"
  critérios/refs: AC-072 (Truncamento seguro de blocos de contexto respeitando limites sintáticos), AC-073 (Deduplicação semântica normalizada antes do empacotamento)
  arquivos permitidos (e seus testes): src/orchestration/context-builder.ts, src/orchestration/ranking.ts, test/p0-memory-dedup-cleanup.spec.test.js
  mensagem de commit: "T-042 p0-memory-dedup-cleanup: Implementar truncamento seguro de blocos e deduplicação no Context Builder"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-042 p0-memory-dedup-cleanup: Implementar truncamento seguro de blocos e deduplicação no Context Builder (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/p0-memory-dedup-cleanup/executar-tarefas.sh --seq T-042"
  FALHAS="$FALHAS T-042"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/p0-memory-dedup-cleanup/executar-tarefas.sh --gate"
      exit 0
    fi
