#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano p5-consolidation-governance-reconciliation` em 2026-08-27 18:33
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo p5-consolidation-governance-reconciliation
set -u
set -o pipefail

RUN_ID='mega-brain-mcp-p5-consolidation-governance-reconciliation-mtbv2wag'
FEATURE='p5-consolidation-governance-reconciliation'
BASE_BRANCH='spec/p5-consolidation-governance-reconciliation'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/p5-consolidation-governance-reconciliation/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p5-consolidation-governance-reconciliation-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-p5-consolidation-governance-reconciliation"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/p5-consolidation-governance-reconciliation/executar-tarefas.sh --faixa $1"
...
# ── faixa-1: T-055 T-056 T-057 ──
executar_faixa_1() {
  local WT="$WT_BASE-faixa-1"
  preparar_worktree 'faixa-1' 'spec/p5-consolidation-governance-reconciliation-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-055' 'Você executa UMA tarefa da feature "p5-consolidation-governance-reconciliation" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p5-consolidation-governance-reconciliation/spec.md, .spec/features/p5-consolidation-governance-reconciliation/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-055 — "Implementar motor determinístico de consolidação e supersessão de memórias"
  critérios/refs: AC-089 (Agrupamento e síntese determinística de memórias do mesmo escopo)
  arquivos permitidos (e seus testes): src/learning/consolidation.ts, src/provenance/repository.ts, test/p5-consolidation-governance-reconciliation.spec.test.js
  mensagem de commit: "T-055 p5-consolidation-governance-reconciliation: Implementar motor determinístico de consolidação e supersessão de memórias"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-056' 'Você executa UMA tarefa da feature "p5-consolidation-governance-reconciliation" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p5-consolidation-governance-reconciliation/spec.md, .spec/features/p5-consolidation-governance-reconciliation/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-056 — "Implementar expurgo de governança para arquivos deletados no Git"
  critérios/refs: AC-090 (Expurgo de memórias vinculadas a arquivos removidos no Git)
  arquivos permitidos (e seus testes): src/lifecycle/governance.ts, src/hooks/dispatcher.ts, src/server/application.ts, test/p5-consolidation-governance-reconciliation.spec.test.js
  mensagem de commit: "T-056 p5-consolidation-governance-reconciliation: Implementar expurgo de governança para arquivos deletados no Git"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-057' 'Você executa UMA tarefa da feature "p5-consolidation-governance-reconciliation" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/p5-consolidation-governance-reconciliation/spec.md, .spec/features/p5-consolidation-governance-reconciliation/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-057 — "Implementar reconciliação em lote de memórias POSSIBLY_STALE"
  critérios/refs: AC-091 (Reconciliação proativa de integridade de AST)
  arquivos permitidos (e seus testes): src/lifecycle/revalidation.ts, src/provenance/freshness.ts, src/server/application.ts, test/p5-consolidation-governance-reconciliation.spec.test.js
  mensagem de commit: "T-057 p5-consolidation-governance-reconciliation: Implementar reconciliação em lote de memórias POSSIBLY_STALE"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/p5-consolidation-governance-reconciliation-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-055 T-056 T-057
  return 0
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/p5-consolidation-governance-reconciliation/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-055, T-056, T-057"
  echo
