#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano agentmemory-runtime-config` em 2026-08-24 20:39
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo agentmemory-runtime-config
set -u
set -o pipefail
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/agentmemory-runtime-config/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-agentmemory-runtime-config-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/mega-brain-mcp-agentmemory-runtime-config"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/agentmemory-runtime-config/executar-tarefas.sh --faixa $1"
...
executar_seq_T_017() {
  info 'sequencial T-017 — Modelar perfis remoto e gerenciado'
  if rodar_tarefa seq 'T-017' 'Você executa UMA tarefa da feature "agentmemory-runtime-config" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/agentmemory-runtime-config/spec.md, .spec/features/agentmemory-runtime-config/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-017 — "Modelar perfis remoto e gerenciado"
  critérios/refs: AC-026 (Modo remoto usa somente conexão), AC-027 (Modo gerenciado recebe configuração nativa segura), AC-028 (Recursos remotos e com LLM exigem opt-in)
  arquivos permitidos (e seus testes): src/config/schema.ts, src/config/load.ts, tests/unit/config.test.ts, .env.example, docs/configuration.md
  mensagem de commit: "T-017 agentmemory-runtime-config: Modelar perfis remoto e gerenciado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-017 agentmemory-runtime-config: Modelar perfis remoto e gerenciado (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/agentmemory-runtime-config/executar-tarefas.sh --seq T-017"
  FALHAS="$FALHAS T-017"
  return 1
}
...
executar_seq_T_018() {
  info 'sequencial T-018 — Propagar configuração somente ao runtime gerenciado'
  if rodar_tarefa seq 'T-018' 'Você executa UMA tarefa da feature "agentmemory-runtime-config" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/agentmemory-runtime-config/spec.md, .spec/features/agentmemory-runtime-config/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-018 — "Propagar configuração somente ao runtime gerenciado"
  critérios/refs: AC-026 (Modo remoto usa somente conexão), AC-027 (Modo gerenciado recebe configuração nativa segura), AC-028 (Recursos remotos e com LLM exigem opt-in)
  arquivos permitidos (e seus testes): src/cli/index.ts, src/cli/install.ts, src/cli/start.ts, src/cli/upgrade.ts, src/runtime/lock-manifest.ts, src/runtime/supervisor.ts, src/runtime/types.ts, tests/integration/runtime-manager.test.ts, tests/e2e/lifecycle.test.ts
  mensagem de commit: "T-018 agentmemory-runtime-config: Propagar configuração somente ao runtime gerenciado"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-sol' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-018 agentmemory-runtime-config: Propagar configuração somente ao runtime gerenciado (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/agentmemory-runtime-config/executar-tarefas.sh --seq T-018"
  FALHAS="$FALHAS T-018"
  return 1
}
...
executar_seq_T_019() {
  info 'sequencial T-019 — Certificar Node 22.22+ e atualizar a matriz'
  if rodar_tarefa seq 'T-019' 'Você executa UMA tarefa da feature "agentmemory-runtime-config" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/agentmemory-runtime-config/spec.md, .spec/features/agentmemory-runtime-config/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-019 — "Certificar Node 22.22+ e atualizar a matriz"
  critérios/refs: AC-029 (Node 22.22 é o piso da matriz suportada)
  arquivos permitidos (e seus testes): package.json, package-lock.json, README.md, docs/troubleshooting.md, .github/workflows/ci.yml, .github/workflows/release.yml, tests/contract/node-compatibility.test.ts
  mensagem de commit: "T-019 agentmemory-runtime-config: Certificar Node 22.22+ e atualizar a matriz"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'gpt-5.6-terra' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-019 agentmemory-runtime-config: Certificar Node 22.22+ e atualizar a matriz (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/agentmemory-runtime-config/executar-tarefas.sh --seq T-019"
  FALHAS="$FALHAS T-019"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/agentmemory-runtime-config/executar-tarefas.sh --gate"
      exit 0
    fi
