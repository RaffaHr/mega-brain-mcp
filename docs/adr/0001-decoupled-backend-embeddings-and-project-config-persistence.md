...
2. **Supported CRG Providers**: Expose `local` (`all-MiniLM-L6-v2`), `openai` (`CRG_OPENAI_*`), `voyage` (`VOYAGE_API_KEY`, `CRG_VOYAGE_*`), `google` (`GOOGLE_API_KEY`), and `minimax` (`MINIMAX_API_KEY`).
